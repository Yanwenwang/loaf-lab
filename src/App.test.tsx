import { fireEvent, render, screen, within } from '@testing-library/react';
import { App } from './App';

describe('App', () => {
  it('renders homepage by default', () => {
    window.history.pushState({}, '', '/');
    render(<App />);

    expect(screen.getByRole('banner')).toBeInTheDocument();
    expect(screen.getByRole('heading', { name: /bake with/i })).toBeInTheDocument();
    expect(screen.getByRole('img', { name: /fresh baked sourdough loaf/i })).toBeInTheDocument();
    expect(screen.getByText(/built with react · typescript · node.js · and a lot of sourdough/i)).toBeInTheDocument();
  });

  it('shows Advisor page after clicking nav link', async () => {
    window.history.pushState({}, '', '/');
    render(<App />);

    const desktopNav = screen.getByTestId('desktop-navigation');
    fireEvent.click(within(desktopNav).getByRole('link', { name: /^advisor$/i }));

    expect(await screen.findByRole('heading', { name: /real answers from/i })).toBeInTheDocument();
    expect(await screen.findByLabelText(/advisor input/i)).toBeInTheDocument();
  });

  it('renders calculator, gallery, and about pages', async () => {
    window.history.pushState({}, '', '/calculator');
    const calculatorView = render(<App />);
    expect(await screen.findByRole('heading', { name: /dough calculator/i })).toBeInTheDocument();
    expect(await screen.findByLabelText(/^total dough weight$/i)).toBeInTheDocument();
    calculatorView.unmount();

    window.history.pushState({}, '', '/gallery');
    const galleryView = render(<App />);
    expect(await screen.findByText(/bake log/i)).toBeInTheDocument();
    expect(await screen.findByRole('article', { name: /hard red wheat, 75% hydration/i })).toBeInTheDocument();
    galleryView.unmount();

    window.history.pushState({}, '', '/about');
    render(<App />);
    expect(await screen.findByRole('heading', { name: /hi, i'm yanwen/i })).toBeInTheDocument();
    expect(await screen.findByText(/official loaf inspector/i)).toBeInTheDocument();
  });

  it('redirects unknown routes to home', () => {
    window.history.pushState({}, '', '/not-a-route');
    render(<App />);

    expect(screen.getByRole('heading', { name: /bake with/i })).toBeInTheDocument();
  });
});
